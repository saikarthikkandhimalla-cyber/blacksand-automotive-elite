...
const blacksandLogo = "/__l5e/assets-v1/7dc5c166-2e68-430e-91f7-8de1a6d88a79/blacksand-logo.jpg";
    ...
    <section id="top" ref={ref} className="relative min-h-screen overflow-hidden pt-24">
      ...
      <div className="relative mx-auto max-w-7xl px-4 sm:px-6 pt-12 sm:pt-24 pb-32">
        <motion.div
          ...
          className="inline-flex items-center gap-2 rounded-full glass px-4 py-2 text-xs uppercase tracking-[0.3em] text-[var(--silver)]"
        >
        ...
        <motion.h1
          ...
          className="mt-6 text-5xl sm:text-7xl md:text-8xl font-bold leading-[0.95] max-w-5xl"
        >
        ...
        <motion.p
          ...
          className="mt-8 max-w-2xl text-lg sm:text-xl text-muted-foreground"
        >
        ...
        <motion.div
          ...
          className="absolute bottom-8 inset-x-0 overflow-hidden border-y border-white/5 py-3"
        >
          ...
          {STATS.map((s, i) => (
            ...
            >
              ...
              <div className="relative">
                <div className="text-4xl sm:text-5xl md:text-6xl font-display font-bold text-gradient-silver">
  ...
  return (
    <section id="services" className="relative py-24 sm:py-32">
      <div className="mx-auto max-w-7xl px-4 sm:px-6">
        <SectionTitle
          kicker="Our Craft"
          title="Services engineered for perfection."
          subtitle="Each treatment is performed by certified detailers using premium global products."
        />
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-5">
          {SERVICES.map((s, i) => (
            <motion.div
              key={s.title}
              initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: (i % 4) * 0.08 }}
            ...
            >
              ...
              <div className="relative">
                <div className="h-12 w-12 rounded-xl glass grid place-items-center mb-5 group-hover:ring-glow transition-shadow">
                  <s.icon className="h-5 w-5 text-primary" />
                </div>
                <h3 className="text-lg font-semibold">{s.title}</h3>
                <p className="mt-2 text-sm text-muted-foreground leading-relaxed">{s.desc}</p>
                <a href="#book" className="mt-5 inline-flex items-center gap-1.5 text-xs font-semibold uppercase tracking-[0.2em] text-primary group/btn">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {WHY.map((w, i) => (
            <motion.div
              ...
              className="glass-strong rounded-2xl p-7 text-center group hover:border-primary/40 transition-colors"
            >
              <div className="mx-auto h-14 w-14 rounded-2xl grid place-items-center mb-5 transition-transform group-hover:scale-110" style={{ background: "var(--gradient-blue)" }}>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-6">
            {STEPS.map((s, i) => (
              <motion.div
                ...
                className="relative"
              >
                <div className="relative z-10 mx-auto h-24 w-24 rounded-full glass-strong grid place-items-center ring-glow">
                  <span className="text-2xl font-display font-bold text-gradient-blue">{s.n}</span>
                </div>
...
const carsQuery = `*[_type == "car"] | order(_createdAt desc){_id, name, price, description, image}`;
const fetchCars = async () => {
  const response = await fetch(`/api/sanity/cars`);
  if (!response.ok) {
    throw new Error("Failed to fetch Sanity cars");
  }
  return (await response.json()) as CarDoc[];
};
        ...
        <div className="flex flex-wrap justify-center gap-2 mb-10">
          {CATEGORIES.map(c => (
            <button
              key={c}
              onClick={() => setActive(c)}
              className={`rounded-full px-5 py-2 text-xs uppercase tracking-[0.2em] transition-all ${active === c ? "bg-primary text-primary-foreground" : "glass hover:border-primary/40"}`}
            >
              {c}
            </button>
          ))}
...
function CarShowcase() {
  ...
  return (
    <section id="cars" className="relative py-24 sm:py-32 bg-background/80">
      <div className="mx-auto max-w-7xl px-4 sm:px-6">
        <SectionTitle
          kicker="Live Cars"
          title="Cars from Sanity Studio"
          subtitle="Any car added in the studio shows up here automatically."
        />

        {isLoading ? (
          <div className="text-center text-muted-foreground">Loading cars…</div>
        ) : isError ? (
          <div className="text-center text-destructive">Failed to load cars from Sanity.</div>
        ) : cars && cars.length > 0 ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {cars.map(car => (
              <article key={car._id} className="overflow-hidden rounded-3xl glass-strong shadow-xl">
                <div className="h-72 overflow-hidden bg-slate-900">
                  <img
                    src={car.image ? urlFor(car.image).width(1200).auto("format").url() : heroCar}
                    alt={car.name ?? "Car image"}
                    className="h-full w-full object-cover transition-transform duration-500 group-hover:scale-105"
                  />
                </div>
                <div className="p-6">
                  <div className="text-xs uppercase tracking-[0.3em] text-primary mb-2">Sanity car</div>
                  <h3 className="text-2xl font-semibold">{car.name ?? "Untitled car"}</h3>
                  {car.price != null && (
                    <p className="mt-2 text-sm text-muted-foreground">KSh {car.price.toLocaleString()}</p>
                  )}
                  {car.description && (
          ...
          <div className="text-center text-muted-foreground">No cars found in Sanity yet.</div>
        )}
      </div>
    </section>
  );
        ...
        <div className="relative glass-strong rounded-3xl p-8 sm:p-14 shadow-luxe overflow-hidden">
          ...
          <AnimatePresence mode="wait">
            ...
            >
              ...
              <div className="mt-8 flex items-center gap-4">
                <div className="h-12 w-12 rounded-full grid place-items-center font-display font-bold" style={{ background: "var(--gradient-blue)" }}>
                  {REVIEWS[idx].name.charAt(0)}
                </div>
                <div>
                  <div className="font-semibold">{REVIEWS[idx].name}</div>
                  <div className="text-xs uppercase tracking-[0.2em] text-muted-foreground">{REVIEWS[idx].car}</div>
                </div>
              </div>
            </motion.div>
          </AnimatePresence>
          <div className="mt-10 flex items-center justify-between">
            <div className="flex gap-1.5">
              {REVIEWS.map((_, i) => (
                <button key={i} onClick={() => setIdx(i)} className={`h-1.5 rounded-full transition-all ${i === idx ? "w-8 bg-primary" : "w-1.5 bg-white/20"}`} />
              ))}
            </div>
            <div className="flex gap-2">
              <button onClick={prev} aria-label="Previous" className="h-10 w-10 grid place-items-center rounded-full glass hover:border-primary/50">
                <ChevronLeft className="h-4 w-4" />
              </button>
              <button onClick={next} aria-label="Next" className="h-10 w-10 grid place-items-center rounded-full glass hover:border-primary/50">
...
function InstagramFeed() {
  ...
  return (
    <section className="relative py-24 sm:py-32 bg-card/40">
      <div className="mx-auto max-w-7xl px-4 sm:px-6">
        <SectionTitle
          kicker="@blacksandauto"
          title="Follow the craft on Instagram."
          subtitle="Daily transformations, behind-the-scenes and detailing tips."
        />
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-2 sm:gap-3">
          {images.map((src, i) => (
            <a key={i} href="https://instagram.com/blacksandauto" target="_blank" rel="noreferrer" className="group relative aspect-square overflow-hidden rounded-xl">
              ...
              <div className="absolute inset-0 bg-background/70 opacity-0 group-hover:opacity-100 transition-opacity grid place-items-center">
          ...
          <div className="glass-strong rounded-3xl p-8 sm:p-10 flex flex-col">
            ...
            <div className="mt-6 space-y-5 text-sm">
              <div className="flex gap-4">
                <div className="h-10 w-10 shrink-0 grid place-items-center rounded-xl glass"><MapPin className="h-4 w-4 text-primary" /></div>
                <div>
                  <div className="font-semibold">Location</div>
                  <div className="text-muted-foreground">DDC Milimani, nearby Milimani City</div>
                </div>
              </div>
              <div className="flex gap-4">
                <div className="h-10 w-10 shrink-0 grid place-items-center rounded-xl glass"><Clock className="h-4 w-4 text-primary" /></div>
                <div>
                  <div className="font-semibold">Working Hours</div>
                  <div className="text-muted-foreground">Mon – Sat: 8 AM – 9 PM</div>
                  <div className="text-muted-foreground">Sunday: 10 AM – 9 PM</div>
                </div>
              </div>
            </div>
            <div className="mt-8 grid grid-cols-1 sm:grid-cols-3 gap-3">
              <a href={PHONE} className="inline-flex items-center justify-center gap-2 rounded-full bg-primary text-primary-foreground px-5 py-3 text-sm font-semibold hover:shadow-[0_0_30px_-5px_var(--primary)] transition-shadow">
                <Phone className="h-4 w-4" /> Call Now
              </a>
              <a href={WHATSAPP} target="_blank" rel="noreferrer" className="inline-flex items-center justify-center gap-2 rounded-full glass px-5 py-3 text-sm font-semibold hover:border-primary/50">
                <MessageCircle className="h-4 w-4" /> WhatsApp
              </a>
              <a href={MAPS} target="_blank" rel="noreferrer" className="inline-flex items-center justify-center gap-2 rounded-full glass px-5 py-3 text-sm font-semibold hover:border-primary/50">
            ...
            <div className="mt-8 rounded-2xl overflow-hidden border border-white/10 aspect-[16/10]">
              <iframe
                title="Map"
                src="https://www.openstreetmap.org/export/embed.html?bbox=36.80%2C-1.30%2C36.83%2C-1.27&layer=mapnik"
          ...
          >
            ...
            <div className="mt-8 grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <select name="service" className="mt-2 w-full bg-transparent border border-white/10 rounded-xl px-4 py-3 text-sm focus:border-primary focus:outline-none">
              ...
              <div className="sm:col-span-2">
                ...
                <textarea name="message" rows={4} className="mt-2 w-full bg-transparent border border-white/10 rounded-xl px-4 py-3 text-sm focus:border-primary focus:outline-none resize-none" />
              </div>
            </div>
            <button type="submit" className="mt-8 w-full inline-flex items-center justify-center gap-2 rounded-full bg-primary text-primary-foreground px-6 py-4 text-sm font-semibold hover:shadow-[0_0_40px_-5px_var(--primary)] transition-shadow">
              {sent ? "Request Sent — We'll Be In Touch" : "Request Appointment"} <ArrowRight className="h-4 w-4" />
            </button>
          </form>
        </div>
      </div>
  ...
  return (
    <div className={full ? "sm:col-span-2" : ""}>
      ...
      <input
        ...
        className="mt-2 w-full bg-transparent border border-white/10 rounded-xl px-4 py-3 text-sm focus:border-primary focus:outline-none"
      />
    </div>
  );
}
...
function FloatingButtons() {
  return (
    <div className="fixed bottom-5 right-5 z-40 flex flex-col gap-3">
      <a href={WHATSAPP} target="_blank" rel="noreferrer" aria-label="WhatsApp" className="h-12 w-12 grid place-items-center rounded-full bg-[#25D366] text-white shadow-luxe hover:scale-110 transition-transform">
        <MessageCircle className="h-5 w-5" />
      </a>
      <a href={PHONE} aria-label="Call" className="h-12 w-12 grid place-items-center rounded-full bg-primary text-primary-foreground shadow-luxe hover:scale-110 transition-transform">
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-10">
          <div className="col-span-2">
            ...
            <p className="mt-5 max-w-sm text-sm text-muted-foreground">
              Premium car detailing and protection studio in Nairobi. Restore, protect and elevate.
            </p>
          ...
          <div>
            ...
            <ul className="space-y-2 text-sm text-muted-foreground">
              {SERVICES.slice(0, 5).map(s => <li key={s.title}><a href="#services" className="hover:text-foreground transition-colors">{s.title}</a></li>)}
            </ul>
          </div>
          <div>
        ...
        <div className="mt-12 pt-6 border-t border-white/5 flex flex-col sm:flex-row justify-between gap-4 text-xs text-muted-foreground">
          ...
          <div className="flex items-center gap-2">
            <CheckCircle2 className="h-3.5 w-3.5 text-primary" /> Crafted in Nairobi
          </div>
        </div>
      </div>
    </footer>
